...
def generate_indices(args, batchsize, mix_time):
    indices = torch.zeros(mix_time * batchsize).to(args.device)
    for i in range(mix_time):
        while True:
            index = torch.randperm(batchsize).to(args.device)
            if not (index == torch.arange(index.size(0)).to(args.device)).any():
...
def generate_linear_interpolate_data(args, code_vec_ori, nl_vec_ori, labels):
    uniform = torch.distributions.uniform.Uniform(0.90, 1.10)
    l = uniform.sample((args.mix_time * code_vec_ori.size(0), 1)).to(code_vec_ori.device)

    nl_index = generate_indices(args, code_vec_ori.size(0), args.mix_time)
    code_index = generate_indices(args, code_vec_ori.size(0), args.mix_time)
    rand_code, rand_nl = code_vec_ori[code_index], nl_vec_ori[nl_index]

    code_vec = torch.cat([code_vec_ori] * args.mix_time, dim=0)
    nl_vec = torch.cat([nl_vec_ori] * args.mix_time, dim=0)
    ...
    labels = torch.cat([labels, torch.ones(combined_code.size(0)).to(args.device)], dim=0)

    return nl_vec, code_vec, labels


def generate_binary_interpolate_data(args, code_vec_ori, nl_vec_ori, labels):
    l = 0.25
    uniform = torch.distributions.uniform.Uniform(0, 1)
    tmp = uniform.sample((1, code_vec_ori.size(1))).to(args.device)
    pos_mask = (tmp > l).int()
    neg_mask = (~ (tmp > l)).int()
...
def generate_dropout_data(args, code_vec_ori, nl_vec_ori, labels):
    code_vec = torch.cat([code_vec_ori] * args.mix_time, dim=0)
    nl_vec = torch.cat([nl_vec_ori] * args.mix_time, dim=0)

    dropout = torch.nn.Dropout(p=0.1)
    combined_code = dropout(code_vec)
    combined_nl = dropout(nl_vec)
...
def generate_scaling_data(args, code_vec_ori, nl_vec_ori, labels):
    code_vec = torch.cat([code_vec_ori] * args.mix_time, dim=0)
    nl_vec = torch.cat([nl_vec_ori] * args.mix_time, dim=0)
...
def train(args, model, tokenizer,pool):
    ...
    for idx in range(args.num_train_epochs): 
        for step,batch in enumerate(train_dataloader):
            ...
            labels = torch.ones(bs, device=code_inputs.device)
            if args.ra:
                aug_index = torch.randint(0, 4, (1,))
                if aug_index == 0:
                    nl_vec, code_vec, labels = generate_linear_interpolate_data(args, code_vec, nl_vec, labels)
                elif aug_index == 1:
                    nl_vec, code_vec, labels = generate_binary_interpolate_data(args, code_vec, nl_vec, labels)
                elif aug_index == 2:
                    nl_vec, code_vec, labels = generate_dropout_data(args, code_vec, nl_vec, labels)
                elif aug_index == 3:
                    nl_vec, code_vec, labels = generate_scaling_data(args, code_vec, nl_vec, labels)

                matrix_labels = torch.diag(labels).float()  # (Batch, Batch)
                matrix_labels = generate_soft_labels(matrix_labels, batch_size=bs,
                                                      mix_time=args.mix_time)

            #calculate scores and loss
            scores=torch.einsum("ab,cb->ac",nl_vec,code_vec)

            if args.ra:
                neg_index = (matrix_labels == 0).nonzero().view(-1, (bs - 1) * (args.mix_time + 1), 2).repeat(1,
                                                                                                              args.mix_time + 1,
                                                                                                              1).view(-1, (
                            bs - 1) * (args.mix_time + 1), 2)
                neg_index = neg_index.view(-1, 2)
                neg_logits = scores[neg_index[:, 0], neg_index[:, 1]].view(-1, (bs - 1) * (args.mix_time + 1))
            ...
            if args.ra:
                loss = loss_fct(scores, torch.zeros(scores.size(0), device=scores.device).long())
            else:
...
def main():
    ...
    parser.add_argument("--mix_time", default=1, type=int,
                        help="The number of tmix times.")
    parser.add_argument("--ra", action='store_true',
                        help="Whether to use representation-level augmentation.")
