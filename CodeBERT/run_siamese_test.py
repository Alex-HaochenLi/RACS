...
from models import ModelContra
...
class InputFeatures(object):
    ...
    def __init__(self, code_tokens, code_ids, nl_tokens, nl_ids, label, idx, code_len, query_len, url):
...
def convert_examples_to_features(js, tokenizer, args, len_list):
    # label
    label = js['label']
    ...
    return InputFeatures(code_tokens, code_ids, nl_tokens, nl_ids, label, js['idx'], code_len, query_len, js['url'])
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None):
        ...
        for idx, js in enumerate(self.data):
            ...
            self.examples.append(convert_examples_to_features(js, tokenizer, args, self.lens))

        # 如果是training集，就print前1个
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in tqdm(range(args.start_epoch, int(args.num_train_epochs))):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            model.train()
            loss, predictions = model(code_inputs, nl_inputs, labels, code_lens)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        mrr = retrieval_CSN(args, model, tokenizer, phase='valid')
...
def retrieval_CSN(args, model, tokenizer, phase):
    ...
    if phase == 'valid':
        q_retrieval_dataset = RetrievalDatasetForCleanCSN(tokenizer, args, '../data/{}/valid.jsonl'.format(args.lang))
    elif phase == 'test':
        q_retrieval_dataset = RetrievalDatasetForCleanCSN(tokenizer, args,
                                                          '../data/{}/test.jsonl'.format(args.lang))
    ...
    c_retrieval_dataset = RetrievalDatasetForCleanCSN(tokenizer, args, '../data/{}/codebase.jsonl'.format(args.lang))
    args.eval_batch_size = args.per_gpu_retrieval_batch_size * max(1, args.n_gpu)
...
def main():
    ...
    if args.checkpoint_path:
        ...
        if torch.cuda.is_available():
            ...
            model.load_state_dict(torch.load(os.path.join(args.checkpoint_path, 'pytorch_model.bin'), map_location=torch.device('cpu')))
    ...
    if args.do_retrieval and args.local_rank in [-1, 0]:
        ...
        tokenizer = tokenizer.from_pretrained(output_dir)
        model.to(args.device)
        mrr = retrieval_CSN(args, model, tokenizer, phase='test')
